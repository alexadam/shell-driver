 ...
 <head>
    ...
    * { box-sizing: border-box; font-family: sans-serif; font-size: 16px;}
    ...
    h1 {font-size: 24px;}
    h2 {font-size: 18px;}
    p {width: 650px; line-height:20px; margin-top:0px; margin-bottom:10px;}
 
     .rd-template {
       display: flex; flex-direction: row; width: 650px; margin-bottom: 12px;
     }
     .rd-inline-template {
      display: inline;
    }
 ...
 <body>
   ...
   <script>
     ...
     function setState(key, value) {
      state[key] = value
      replaceAllTemplates()
      window.top.postMessage({newState: state}, '*')
     }
     ...
     window.onmessage = function(e) {
        if (e.data.updateState) {
          document.getElementById(e.data.updateState.id).value = e.data.updateState.value
          updateState(e.data.updateState)
        }
      };
     ...
     function runTemplate(templateId) {
      ...
      window.top.postMessage({run: content}, '*')
    }
     ...
     function replaceAllTemplates() {
       ...
       const allShellElems = document.getElementsByClassName('rd-shell-content')
